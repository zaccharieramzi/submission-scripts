...
executor = get_executor('ncmc_tfrecords', timeout_hour=20, n_gpus=1, project='fastmri4', no_force_32=True)
with executor.batch():
    # for mode in ['train', 'val']:
    for mode in ['val']:
        for acq_type in ['radial', 'spiral']:
            for af in [4, 8]:
                executor.submit(generate_multicoil_nc_tf_records, acq_type=acq_type, af=af, mode=mode, brain=True)
